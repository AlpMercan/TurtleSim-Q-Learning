...
def run_simulation(q_table_path="q_table.pkl"):
    # Load the Q-table from file
    with open(q_table_path, "rb") as f:
        q_table = pickle.load(f)

    state = (0, 0)  # Starting position of the turtle
    robot.goto(0, 0)

    while not check_target():  # Continue until the target is reached
        action = choose_best_action(state, q_table)
        move_turtle(action)
        state = (int(robot.xcor()), int(robot.ycor()))
